...
class Conv_Decomp(nn.Module):
    def __init__(self, num_filters, filter_h, filter_w, image_channels, rank, devices):
        ...
        weights, factors = parafac(tensor, rank)
         # Assign weights and factors as parameters
        self.weights = nn.Parameter(torch.tensor(weights).to(devices[0]))
        ...
        #     nn.Parameter(torch.tensor(factors[0]).to(devices[0])),
        #     nn.Parameter(torch.tensor(factors[1]).to(devices[0])),
        #     nn.Parameter(torch.tensor(factors[2]).to(devices[0])),
        #     nn.Parameter(torch.tensor(factors[3]).to(devices[0]))
        # ])
