...
class DecompLayer(nn.Module):
    def __init__(self, num_filters, filter_h, filter_w, original_channels, rank):
        ...
        #     print(factor.shape)
        # factors = [np.random.randn(num_filters, rank), 
        #            np.random.randn(filter_h, rank), 
        #            np.random.randn(filter_w, rank),
        #            np.random.randn(original_channels, rank)]
    ...
    def forward(self, x):
        ...
        x = x.unfold(2, self.filter_h, 1).unfold(3, self.filter_w, 1).contiguous()
        shape = x.shape
        x = x.reshape(shape[0], shape[1], shape[2]*shape[3], shape[4],shape[5])#B,C,N,h,w
        x = x.permute(0, 2, 3, 4, 1)#B,N,h,w,C
        # output = torch.tensor(np.zeros((shape[0], (shape[2])*(shape[3]), self.num_filters)), dtype=x.dtype)
        ...
        with ThreadPoolExecutor(max_workers=self.rank) as executor:
            futures = [executor.submit(self.multi_task, x, i, shape) for i in range(self.rank)]
